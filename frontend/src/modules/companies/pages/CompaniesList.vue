    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p class="mt-1 text-sm text-slate-500">Bağlı olduğunuz organizasyonları buradan yönetebilirsiniz.</p>
        </div>
      ...
      <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <div v-for="n in 8" :key="n" class="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm animate-pulse flex items-center gap-4">
      ...
      <div v-else-if="companies.length === 0" class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-slate-300 text-center">
        ...
        <p class="text-slate-500 text-sm mt-1 max-w-sm">Yeni bir şirket profili oluşturarak projelerinizi ve kullanıcılarınızı yönetmeye başlayın.</p>
        <button @click="goToCreate" class="mt-6 px-6 py-2 bg-white border border-slate-300 text-slate-700 font-semibold rounded-lg hover:bg-slate-50 transition-colors">
           Şirket Oluştur
        </button>
      </div>
