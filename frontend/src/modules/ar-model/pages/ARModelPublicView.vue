<template>
  <div class="mobile-vh w-full bg-gray-900 flex flex-col relative overflow-hidden">
    ...
    <div v-else-if="error"
      class="absolute inset-0 z-50 flex flex-col items-center justify-center bg-gray-50 px-4 text-center">
    ...
    <div v-else class="relative w-full h-full">
      ...
      <div class="w-full h-full bg-gradient-to-br from-gray-800 to-gray-900">
        <ArPreview v-if="previewBlobUrl" :src="previewBlobUrl" class="w-full h-full" format="glb" />
      </div>

      <div class="ar-button-container absolute left-0 right-0 z-30 flex justify-center px-4">
        <button @click="activateAR"
          ...
          <div class="relative w-5 h-5">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 group-hover:animate-bounce" fill="none"
...
// Viewport height ayarlama (mobil tarayıcı URL bar sorunu için)
...
onMounted(async () => {
  // Viewport height ayarla ve event listener'ları ekle
  setViewportHeight();
  window.addEventListener('resize', setViewportHeight);
  window.addEventListener('orientationchange', setViewportHeight);

  // Mobil tarayıcıda URL bar'ını gizlemek için scroll
  setTimeout(() => {
    window.scrollTo(0, 1);
  }, 100);
...
.ar-button-container {
  bottom: 2rem;
  /* Safe area inset desteği (iPhone X ve üzeri için) */
