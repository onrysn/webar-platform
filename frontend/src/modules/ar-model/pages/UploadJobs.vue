<template>
  <div class="p-6">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold tracking-tight">{{ t('models.uploadQueue.title') }}</h1>
        <p class="text-sm text-gray-500">{{ t('models.uploadQueue.subtitle') }}</p>
      </div>
      <div class="flex items-center gap-2">
        <select v-model="statusFilter" class="px-3 py-2 text-sm rounded-xl border border-gray-300 bg-white">
          <option value="">{{ t('common.all') }}</option>
          <option value="QUEUED">{{ t('models.uploadQueue.waiting') }}</option>
          <option value="CONVERTING">{{ t('models.uploadQueue.converting') }}</option>
          <option value="CONVERTED">{{ t('models.uploadQueue.completed') }}</option>
          <option value="APPROVED">{{ t('models.uploadQueue.approved') }}</option>
          <option value="ERROR">{{ t('models.uploadQueue.failed') }}</option>
        </select>
        <button class="px-3 py-2 text-sm rounded-xl border border-gray-300 bg-white hover:bg-gray-50" @click="refresh">{{ t('common.refresh') }}</button>
      </div>
    </div>

    <div v-if="isSuperAdmin" class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-2xl">
      <div class="flex items-center gap-3">
        <span class="text-sm font-semibold text-amber-900">{{ t('models.filterByCompany') }}:</span>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      <div v-for="job in jobs" :key="job.tempId" class="bg-white border border-gray-200 rounded-2xl shadow-sm p-4">
        <div class="flex items-center justify-between mb-2">
          ...
          <span :class="statusClass(job.status)" class="px-2 py-1 rounded text-[10px] font-semibold">{{ job.status === 'APPROVED' ? t('models.uploadQueue.approved') : job.status }}</span>
        </div>
        ...
        <div class="mt-2">
          ...
          <div class="mt-1 text-[11px] text-gray-500">{{ t('models.uploadQueue.progress') }}: {{ Math.round(job.progress || 0) }}%</div>
        </div>
        <div class="mt-3 text-xs text-gray-600 truncate">
          {{ job.status === 'APPROVED' ? t('models.uploadQueue.approved') : (job.message || '-') }}
        </div>
