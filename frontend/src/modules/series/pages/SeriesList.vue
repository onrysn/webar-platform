<template>
  <div class="p-6 space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold text-slate-900">{{ t('series.title') }}</h1>
        <p class="text-xs text-slate-500">{{ t('series.subtitle') }}</p>
      </div>
      <div class="flex items-center gap-2">
        ...
        <button @click="openCreate" class="px-3 py-2 rounded-lg bg-indigo-600 text-white text-sm font-bold">{{ t('series.createNew') }}</button>
      </div>
    </div>
    ...
    <div v-if="isSuperAdmin" class="flex items-center gap-3 bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
      ...
      <button v-if="selectedFilterCompanyId" @click="clearFilter" class="text-xs text-red-600 font-bold hover:underline">{{ t('common.remove') }}</button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      <div v-for="s in filtered" :key="s.id" class="p-4 rounded-2xl bg-white border border-slate-200 hover:shadow-md transition-all flex justify-between items-start">
        <div class="space-y-1">
          ...
          <div class="text-[11px] text-slate-500">{{ t('series.code') }}: {{ s.code || '—' }} · #{{ s.id }}</div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="bg-white p-5 rounded-2xl w-full max-w-md space-y-4 border border-slate-200 shadow-xl">
        <div class="font-bold text-slate-800">{{ editing ? t('series.editSeries') : t('series.addSeries') }}</div>
        <div class="space-y-3">
          <div>
            <label class="block text-[11px] font-bold text-slate-400 uppercase">{{ t('common.name') }}</label>
            <input v-model="form.name" :placeholder="t('series.seriesNamePlaceholder')" class="w-full border rounded-lg px-3 py-2 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none" />
          </div>
          <div>
            <label class="block text-[11px] font-bold text-slate-400 uppercase">{{ t('common.type') }}</label>
            <input v-model="form.code" :placeholder="t('common.optional')" class="w-full border rounded-lg px-3 py-2 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none" />
          </div>
          <div>
            <label class="block text-[11px] font-bold text-slate-400 uppercase">{{ t('common.description') }}</label>
            <input v-model="form.description" :placeholder="t('common.optional')" class="w-full border rounded-lg px-3 py-2 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none" />
          </div>
        </div>
